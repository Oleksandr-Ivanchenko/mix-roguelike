...
export class OreShopMenu {
  ...
  _build() {
    ...
    const sy    = H / 2 - gridH / 2 - 16;

    // Overlay
    this._push(s.add.rectangle(W / 2, H / 2, W, H, 0x000000, 0.90).setScrollFactor(0).setDepth(70));

    // Title
    this._push(s.add.text(W / 2, sy - 46, "🔧  МАГАЗИН РУД", {
    ...
    const invStr = Object.entries(s.ores)
      .filter(([, v]) => v > 0)
      .map(([k, v]) => `${ORE_SHORT[k]}×${v}`)
      .join("   ") || "— руд нет —";
    this._push(s.add.text(W / 2, sy - 18, invStr, {
    ...
    UPGRADES.forEach((upg, i) => {
      ...
      const can  = this._canAfford(upg.cost);

      const bg = s.add.rectangle(mid, by + cardH / 2, cardW, cardH, can ? 0x1a2233 : 0x111118)
        .setScrollFactor(0).setDepth(71)
        .setStrokeStyle(1, can ? 0x4455aa : 0x222233);

      const icon = s.add.text(mid, by + 22, upg.icon, { fontSize: "22px" })
        .setOrigin(0.5).setScrollFactor(0).setDepth(72);

      const lbl = s.add.text(mid, by + 52, upg.label, {
      ...
      const costTxt = s.add.text(mid, by + 84, costStr, {
      ...
      const btnBg = s.add.rectangle(mid, btnY, cardW - 20, 20, can ? 0x2244aa : 0x151520)
        ...
        .setStrokeStyle(1, can ? 0x5566dd : 0x222233);
      const btnTxt = s.add.text(mid, btnY, "КУПИТЬ", {
    ...
    // Close hint
    this._push(s.add.text(W / 2, sy + gridH + 14, "[E] — закрыть магазин", {
